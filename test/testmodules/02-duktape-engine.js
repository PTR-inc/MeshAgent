...
//
// Duktape Engine: the native embedding layer, covering EventEmitter, Duplex streams with the
// '~' finalizer, the promise module and GC.
//
...
exports.run = function (check, deepEqual, done) {
    ...
    // Mixin-style EventEmitter, as this codebase uses it rather than a Node-style new EventEmitter().
    // The true argument selects explicit-events mode, where .on() throws "Cannot register for
    // non-existing event" unless createEvent() declared the name first, as in modules/daemon.js.
    var ee = {};
    ...
    ee.emit('probe', 100); // Must not be counted, because the listener was removed.
    ...
    // Duplex stream with the '~' finalizer event, the same shape as the sample streams in test/leaktest.js.
    var received = '';
    ...
    // GC stress: allocate and drop a lot of objects and buffers, then force a collection pass.
    for (var g = 0; g < 500; ++g) {
    ...
    // Promise chaining, rejection and .catch(). An already-settled executor resolves synchronously
    // here, with no microtask queue involved, so these are checked immediately rather than deferred.
    ...
    // than their unwrapped values in this implementation, so only settling is checked.
    var allResult = null;
    Promise.all([
