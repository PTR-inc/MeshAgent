...
//
// IO: synchronous fs read, write, stat, exists and unlink, streaming read and write, and a
// child_process round-trip.
//
...
exports.run = function (check, deepEqual, done) {
    ...
    function streamRoundTrip(label, path, wopts, ropts, next) {
        ...
        ws.on('finish', function () {
            ...
            // Unlike Node, attaching a 'data' listener alone does not start the flow here. resume()
            // must be called after the listeners are attached, or 'data' and 'end' never fire.
            rs.resume();
        });
    ...
    function testChildProcess(next) {
        try {
            ...
            // Windows CreateProcessW gets lpApplicationName, which does not search PATH, so a bare
