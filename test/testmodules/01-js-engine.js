...
//
// JS Engine - core ECMAScript behavior inside Duktape (closures, exceptions, JSON, RegExp,
// Array/String/Math/Date, Buffer).
//
...
exports.run = function (check, deepEqual, done) {
    ...
    c1(); c1(); c1(); // 3 prior increments on c1, none on c2 - each counter's ++n is call-once, so
    var c1v = c1(), c2v = c2(); // capture each counter's call exactly once here, not inside check()
    ...
    // Buffer (Node-style buffer object). NB: .toString() here only recognizes
    // 'base64'/'hex'/'hex:' (+'utf16' on Windows) as explicit encodings - unlike Node,
    // 'utf8' is NOT one of them; the plain string conversion is the no-argument form.
