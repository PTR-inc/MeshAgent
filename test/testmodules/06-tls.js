...
//
// TLS Connections - repeated local TLS handshakes against a self-signed https server (same
// shape as the known Windows reconnect/hang defects in meshagent-todo.md #1/#2, but
...
exports.run = function (check, deepEqual, done) {
    ...
    // https.createServer()'s emitter only declares checkContinue/checkExpectation/clientError/
    // close/connect/connection/request/upgrade - no plain 'error', unlike the net/tls socket
    // emitters (explicit-events mode throws "Cannot register for non-existing event" otherwise).
    ...
    // Per-attempt guard timer, anchored on this outer var (see meshagent-todo.md #0d) so it
    // isn't collected before it can fire. It only protects against a silent hang (the known
    // second-sequential-connection defect, #2) - a crash inside the connect/end() cycle itself
    // (the known reconnect-after-end() defect, #1) kills the whole process outright and no
    // JS-level guard can catch that; wrap invocation with an external `timeout` for that case.
