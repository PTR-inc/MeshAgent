...
# ---------------------------------------------------------------- OpenSSL ----
# openssl/VERSION is the only pin, read by this script, the makefile and the MSBuild props.
# An environment override selects another installed prefix under openssl/<version>/.
# fetch-toolchains.sh reads the sha256 from openssl.org's own sidecar, so there is no checksum to keep.
export OPENSSL_VERSION="${OPENSSL_VERSION:-$(tr -d '[:space:]' < "$REPO/openssl/VERSION")}"
...
ossl_flags_file() {
    ...
    echo "build-env.sh: no flags file for OpenSSL $v under $BR_SCRIPTS/flags/" >&2; return 1
}
export OSSL_FLAGS_FILE="$(ossl_flags_file "$OPENSSL_VERSION")"
...
# "NULLcanary" fills from Apple's hard-link placeholders. See openssl/build/README.md.
osxcross_clone() {
...
br_check() {
    ...
    if [ $missing -ne 0 ]; then
        echo "  see openssl/build/README.md for how to provision \$BUILDROOT ($BUILDROOT)"
    else
