...
br_target() {
    ...
    # Little-endian MIPS on the pinned Bootlin uClibc toolchain, from when ARCHID 7's agent was
    # uClibc and could not link a glibc archive at all. asm runs correct crypto under qemu-mipsel.
    ...
    #
    # Retired 2026-09-05, kept only so a uClibc target can be built on demand: ARCHID 7 went
    # dynamic-uClibc -> static-musl on 2026-08-31 and now links linux-mips32r1el-musl, which left
    # this prefix with no ARCHID at all (`build.sh list-targets` showed ARCHIDS = -) while still
    # costing a CI job and 21 MB of committed archives. So it is out of BR_ALL_TARGETS and out of
    # the CI matrix, and nothing installs it unless it is named explicitly. Note that dropping it
    # does not drop the toolchain: ARCH_7 still fetches bootlin-mipsel-uclibc to supply STRIP.
...
#
# -S (--strip-debug) is not optional on the gcc branch, the same way the macos branch below has
# always had it. GNU strip with no flag means --strip-all, which on a .a takes the symbol table
# out of every member, and a static library with no symbols is dead weight: measured 2026-09-05 on
# linux-sparc64-glibc's libssl.a, plain strip left 385498 bytes carrying 0 of its 985 global
# symbols, and the link failed with "archive has no index; run ranlib to add one". ranlib cannot
# undo it either, since it only rebuilds an index over whatever symbols are left, which is none.
# -S keeps all 985 and still drops 5471000 bytes to 913112.
br_strip_cmd() {
    case "$T_CC" in
        "$TC_ZIG"/zig\ cc*) echo "" ;;
        *-gcc)              echo "${T_CC%-gcc}-strip -S" ;;
