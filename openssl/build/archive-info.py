#!/usr/bin/env python3
"""Print format, wordsize, machine, symbol-index presence and member count of a static archive.

Walks the ar container itself, so GNU (.a), BSD (Mach-O .a) and COFF (.lib) archives all work
on any host, with no binutils and nothing that can read the target's object format. Output is one
line: format=ELF|MACHO|COFF class=32|64 machine=<name> symindex=yes|no members=<n>.

symindex is what a linker needs to resolve anything out of the archive ("archive has no index;
run ranlib to add one"); members= stays last because probe.sh reads it with a suffix strip.
"""
import struct, sys
...
def main(path):
    ...
    off, seen, members, symindex = 8, {}, 0, False
    while off + 60 <= len(d):
        ...
        if name in ('/', '//', '/SYM64/') or name.startswith('__.SYMDEF'):
            # '//' is the long-name table, not an index. The rest are the symbol index in their
            # respective flavours: '/' for GNU and COFF, '/SYM64/' for 64-bit GNU, '__.SYMDEF'
            # (optionally ' SORTED') for BSD and Mach-O.
            if name != '//':
                symindex = True
            continue
        members += 1
    ...
    print('format=%s class=%d machine=%s symindex=%s members=%d'
          % (fmt, cls, mach, 'yes' if symindex else 'no', members))
