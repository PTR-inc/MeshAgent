...
probe_archive() {
    ...
    P_SYMINDEX=${info#*symindex=}; P_SYMINDEX=${P_SYMINDEX%% *}
...
gate_target() {
    ...
    # Without an index the archive links nothing at all, so this comes before the finer checks.
    # It is the one failure the strings- and container-based gates below cannot see: they all still
    # pass on an archive whose symbol table has been stripped out, which is how a --strip-all'd
    # libssl.a once shipped and only failed at agent link time, one target and many commits later.
    [ "$P_SYMINDEX" = yes ] || reject "archive has no symbol index (a --strip-all'd .a? strip -S keeps it)"
    ...
    if [ -f "$conf" ]; then
        if [ "${t%-debug}" != linux-sparc64-glibc ]; then
            case "$P_CLASS" in
