...
1. **This directory** (`openssl/libstatic/build/`) — the OpenSSL-archive build scripts themselves
   ...
   fetcher, `provision-buildroot.sh`, lives one level up at the repo root instead — it provisions
...
```sh
./provision-buildroot.sh               # fresh machine: download+verify+extract everything fetchable
...
`provision-buildroot.sh` lives at the repo root, not under `openssl/`, on purpose: `$BUILDROOT`'s
...
`./provision-buildroot.sh` (repo root) automates everything below that has a public URL: the
...
arguments for everything, or name specific components (`provision-buildroot.sh freebsd openbsd`);
`provision-buildroot.sh list` reports status without fetching anything. It also symlinks the two
