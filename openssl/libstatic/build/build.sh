...
[ $# -ge 1 ] || { echo "usage: $(basename $0) <target|all> [target...]"; echo "targets: $BR_ALL_TARGETS"; exit 2; }
...
build_one() {
    ...
    case "$t" in
      mips|riscv64|alpine-x86-64|mips24kc|mipsel24kc|openwrt_x86_64|aarch64-cortex-a53|linux-armada370-hf)
        if [ "$g" -ne 0 ]; then
